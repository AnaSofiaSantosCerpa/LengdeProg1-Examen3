...
*/
public open class DFS(val g: Grafo) : Busqueda(g) {
    val ot : LinkedList<Int> = LinkedList<Int>()
    ...
    */
    ...
    override fun buscar(f: Int, h: Int) : Int {
        //Aplicacion de DFS con la Pila creada para el examen de Lenguajes de Programacion I
        var colores = Array(n){(Color.BLANCO)}
        var s = Pila<Int>(mutableListOf<Int>())
        var v = 0
        s.agregar(v)
        colores[v] = Color.GRIS
        var cont = 0
        if (v == f) {
            cont += 1
        }
        ...
        while (!s.vacio()) {
            v = s.remover()
            g.adyacentes(v).forEach {w ->
                if (colores[w.elOtroVertice(v)] == Color.BLANCO) {
                    if (w.elOtroVertice(v) == f && cont == 0) {
                        cont += 1
                    } else if (cont > 0 && w.elOtroVertice(v) != h) {
                        cont += 1
                    } else if (w.elOtroVertice(v) == h) {
                        cont += 1
                        return cont
                    } else if (f == h && w.elOtroVertice(v) == h) {
                        return 1
                    }
                    colores[w.elOtroVertice(v)] = Color.GRIS
                    s.agregar(w.elOtroVertice(v))
                }
            }
            colores[v] = Color.NEGRO
        }
